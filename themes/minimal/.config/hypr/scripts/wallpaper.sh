...
# Source library.sh
source $HOME/.config/zuabua/scripts/library.sh

# -----------------------------------------------------
# Check to use wallpaper cache
# -----------------------------------------------------

if [ -f ~/.config/zuabua/settings/wallpaper_cache ]; then
    use_cache=1
...
# Create cache folder
# -----------------------------------------------------
zuabua_cache_folder="$HOME/.cache/zuabua/hyprland-dotfiles"

if [ ! -d $zuabua_cache_folder ]; then
    mkdir -p $zuabua_cache_folder
fi
...
generatedversions="$zuabua_cache_folder/wallpaper-generated"
if [ ! -d $generatedversions ]; then
    mkdir -p $generatedversions
fi

# Will be set when waypaper is running
waypaperrunning=$zuabua_cache_folder/waypaper-running
if [ -f $waypaperrunning ]; then
    rm $waypaperrunning
    exit
fi

cachefile="$zuabua_cache_folder/current_wallpaper"
blurredwallpaper="$zuabua_cache_folder/blurred_wallpaper.png"
squarewallpaper="$zuabua_cache_folder/square_wallpaper.png"
rasifile="$zuabua_cache_folder/current_wallpaper.rasi"
blurfile="$HOME/.config/zuabua/scripts/blur.sh"
defaultwallpaper="$HOME/.config/zuabua/wallpapers/default.jpg"
wallpapereffect="$HOME/.config/zuabua/scripts/wallpaper-effect.sh"
blur="50x30"
blur=$(cat $blurfile)
